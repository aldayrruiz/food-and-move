...
@Component({
  ...
  styleUrls: [
    './add-routine-for-week-routine-page.component.css',
...
export class AddRoutineForWeekRoutinePageComponent implements OnInit {
        ...
        .subscribe(
          (res) => {
            ...
            if (params['recipeId']) {
              ...
              this.loaderService.isLoading.next(true);
              this.dietsService
                .getRecipe(this.diet!._id, this.day!, params['recipeId'])
                .pipe(finalize(() => this.loaderService.isLoading.next(false)))
                .subscribe(
                  (res) => {
                    this.recipe = res;
  ...
  initForm(): void {
    ...
    if (this.edit) {
      ...
      this.links =
        this.recipe!.links.map((url, id) => {
          return { id, url };
        }) || [];
      this.videos =
        this.recipe!.videos.map((url, id) => {
          return { id, url };
        }) || [];
      this.ingredients =
        this.recipe!.ingredients.map((ingredient, id) => {
